...
// Search for the career fit page block
...
// Wrap the end of career fit layout in the conditional closing tags:
content = content.replace(
  `                        <button className="w-full text-center text-xs font-black text-indigo-650 border border-slate-200 py-3 rounded-2xl hover:bg-slate-50 transition-colors shadow-sm" onClick={() => { const nextRole = activeCareer?.title; setTargetRole(nextRole); fetchSkillGap(resumeId); }}>
                          Select as Target Career Role
                        </button>
                      </div>
